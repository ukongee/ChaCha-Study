...
"시험 대비 압축 요약"이 아니라 "구조화된 페이지별 해설"이 목표입니다.

[briefSummary — 최초 생성 시에만 포함]
- 문서 전체 흐름과 핵심 주제를 3줄 이내로 요약

[pages 배열]
page: 페이지 번호 (정수, 원문 페이지 번호 그대로)
title: 해당 페이지의 핵심 주제 (10자 이내, 명사형)
summary: 이 페이지 핵심 내용 2~3문장
detailedExplanation: 페이지 내용을 빠짐없이 한국어로 구조화해서 설명
  - 정의 → 특징 → 관계/조건 → 흐름 순으로 자연스럽게 풀어서 작성
  - 페이지에 있는 중요한 내용은 모두 포함
  - 번역투 금지 — 자연스러운 한국어 설명
  - 억지 예시 금지, 정말 필요한 경우에만 짧게
  - 최소 150자 이상
keyTerms: 이 페이지에서 반드시 알아야 할 핵심 용어 (3~7개)

[금지]
- 시험 출제 가능성, 암기 포인트, 중요도 표시 금지
...
{
  "briefSummary": "문서 전체 흐름 요약 (3줄 이내)",
  "pages": [
    {
      "page": 1,
      "title": "페이지 핵심 주제",
      "summary": "핵심 내용 2~3문장",
      "detailedExplanation": "페이지 내용을 구조적으로 한국어 설명",
      "keyTerms": ["용어1", "용어2"]
    }
  ]
}
